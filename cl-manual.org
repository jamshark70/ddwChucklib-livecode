...
in parentheses, and applies it to any existing =BP= process. No syntax
checking is done in the preprocessor, apart from counting parentheses
to know which one really ends the statement.
...
** Chuck statement
The chuck statement is a shortcut for chucking a =BP= into some other object.

...
a 16th. Further, source strings will be compressed to fit into the
generator's duration. If @@latex:\texttt{"|||"}@@ at the outermost
layer produces four divisions of one beat each, the same inside
example 3's =\ins()= generator would divide 2.75 beats by 4, whereupon
each division would consist of 11 64th-notes. These examples avoid the
problem by omitting the bracketed source strings. Otherwise, be
...
- A sequence of items with which to replace a wildcard (or, for rhythm
  ...
  - =\xrand()=
  - =\shuf()=
  See [[genSequencers]] for examples of complex sequences produced by nested generators.

...
- A number. Integer ranges may be specified =low..high=, e.g., =1..3=
  ...
  pass-through expressions, in braces. For example, to "choke" notes
  either 0.25 or 0.5 beats later, you could write
  =\someGenerator(...)::\choke(, {rrand(1,2)*0.25}, "x")=.
...
- =\pitch(src, "wildcards", "intervalPool", fallbackMin, fallbackMax, "articPool")= :: Valid
     ...
     =2= is up a second, =8= is up an octave, and =6,= is a third down
     (e.g., in C major, =6= is up a sixth = A, and =6,= lowers this by
     an octave to be the A a third below the root). If there is no
